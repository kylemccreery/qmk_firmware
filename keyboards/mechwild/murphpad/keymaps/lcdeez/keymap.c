...
void light_off(void) {
    ...
    wait_ms(10);
    #ifdef CONSOLE_ENABLE
        dprintf("tried to turn it off\n");
    #endif
}

void write_LCD_string(char* stringToWrite) {
    int len = strlen(stringToWrite);
    BYTE arr[len];
    int i;

    //converting string to BYTE[]
    string2ByteArray(stringToWrite, arr);
...
void keyboard_post_init_user(void) {
  ...
  screen_init();
  light_on();
  char ascii_str[] = "Keyboards are   fun!";
  write_LCD_string(ascii_str);
}
